class Staff::EntriesForm
  ...
  private def save
    ...
    ActiveRecord::Base.transaction do
      @program.entries.where(id: approved_entry_ids.present?)
        .update_all(approved: true) if approved_entry_ids.present?
      @program.entries.where(id: not_approved_entry_ids.present?)
        .update_all(approved: false) if not_approved_entry_ids.present?
      @program.entries.where(id: canceled_entry_ids.present?)
        .update_all(canceled: true) if canceled_entry_ids.present?
      @program.entries.where(id: not_canceled_entry_ids.present?)
