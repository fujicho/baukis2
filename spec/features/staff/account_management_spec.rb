...
feature "職員によるアカウント管理" do
  ...
  scenario "職員がメールアドレス、氏名、フリガナを更新する" do
    ...
    eppect(staff_member.family_name).to eq("試験")
    eppect(staff_member.given_name).to eq("花子")
    eppect(staff_member.family_name_kana).to eq("シケン")
    eppect(staff_member.given_name_kana).to eq("ハナコ")
  end
